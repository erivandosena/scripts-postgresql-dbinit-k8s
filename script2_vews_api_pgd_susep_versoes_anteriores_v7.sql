...
CREATE OR REPLACE VIEW "VW_ATIVIDADE"
AS 
...
CREATE OR REPLACE VIEW "VW_PACTO_HORAS_HOMOLOGADAS"
AS 
...
from "ProgramaGestao"."PactoTrabalho" as pa 
	...
	inner join dbo."VW_UnidadeSiglaCompleta" as s on pa."unidadeId" = s."unidadeId"
	inner join dbo."CatalogoDominio" AS cd1 on pa."situacaoId" = cd1."catalogoDominioId"
	inner join "ProgramaGestao"."VW_PACTO_HORAS_HOMOLOGADAS" AS vw on vw."IdPacto" = pa."pactoTrabalhoId"
...
SELECT ROW_NUMBER() OVER (ORDER BY pta."pactoTrabalhoAtividadeId" ASC) AS id
    ...
    ,CASE WHEN pt."formaExecucaoId" = 101 -- SQLINES DEMO *** aoId = 102  AND pta.modalidadeExecucaoId = 101) 
    ...
    ,CASE WHEN pt."formaExecucaoId" = 103 -- SQLINES DEMO *** aoId = 102 AND pta.modalidadeExecucaoId = 103)
...
CREATE OR REPLACE VIEW "VW_SITUACAO_PACTO"
AS
...
CREATE OR REPLACE VIEW "VW_TIPO_ATIVIDADE"
AS
